...
const stickerSeeds: StickerSeed[] = [
  ...
  { id: 2, name: 'Vinicius Jr', team: 'Brasil', image: 'https://media.gettyimages.com/id/2171257223/pt/foto/asuncion-paraguay-vinicius-junior-of-brazil-reacts-during-the-south-american-fifa-world-cup.jpg?s=612x612&w=0&k=20&c=-vDlakCN__ARvebJ4_9YVvHxp-wid9ysYW_GVWPn7sY=' },
...
export async function addContato(name: string, email: string, telefone: string) {
  ...
  await getDb().run(query, [name, email, telefone]);
}
...
export async function updateContato(id: number, name: string, email: string, telefone: string) {
  ...
  await getDb().run(query, [name, email, telefone, id]);
}
...
export async function updateUsuario(id: number, nome: string, login: string, senha: string) {
  ...
  return await getDb().run(query, [nome, login, senha, id]);
}

export async function listUsuarios() {
  await ensureDatabase();
  const result = await getDb().query('SELECT id, nome, login FROM usuarios');
  return result.values || [];
}
...
export async function findUsuarioByLogin(login: string) {
  await ensureDatabase();
  const result = await getDb().query('SELECT id, nome, login FROM usuarios WHERE login = ?', [login]);
  return result.values?.[0] || null;
}

async function ensureAlbumSeed(userId: number) {
  await ensureDatabase();
  const countResult = await getDb().query('SELECT COUNT(*) as total FROM album_figurinhas WHERE user_id = ?', [userId]);
  ...
  for (const sticker of stickerSeeds) {
    await getDb().run(
      'INSERT INTO album_figurinhas (user_id, sticker_id, name, team, image, collected) VALUES (?, ?, ?, ?, ?, ?)',
      [userId, sticker.id, sticker.name, sticker.team, sticker.image, 0]
    );
  }
}

export async function listStickersForUser(userId: number, options?: { search?: string; filter?: 'all' | 'collected' | 'pending' }) {
  await ensureDatabase();
  await ensureAlbumSeed(userId);

  const search = options?.search?.trim().toLowerCase() || '';
  const filter = options?.filter || 'all';
  let query = 'SELECT sticker_id AS id, name, team, image, collected FROM album_figurinhas WHERE user_id = ?';
...
export async function toggleStickerCollectedForUser(userId: number, stickerId: number) {
  ...
  const result = await getDb().query('SELECT collected FROM album_figurinhas WHERE user_id = ? AND sticker_id = ?', [userId, stickerId]);
  const currentValue = Number((result.values?.[0] as { collected?: number } | undefined)?.collected || 0) === 1;
  const nextValue = currentValue ? 0 : 1;

  await getDb().run('UPDATE album_figurinhas SET collected = ? WHERE user_id = ? AND sticker_id = ?', [nextValue, userId, stickerId]);
  return nextValue === 1;
}