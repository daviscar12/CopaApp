      ...
      <div class="auth-card-wrap">
        <ion-card class="card-accent auth-card">
          <div class="auth-card-header">
            <h2>Redefinição</h2>
            <p>Vamos enviar instruções ao e-mail cadastrado.</p>
          </div>

          <ion-card-content>
            <ResetPasswordForm @reset="onReset" />
...
function onReset(payload: { email: string }) {
  try {
    resetPassword(payload.email);
    router.push('/login');
    alert('E-mail de redefinição enviado (simulado)');
  } catch (e: any) {
